...
begin
  ...
  begin
    ShowMessage('Este CEP contém todos os dígitos iguais e não é aceito pelas demais APIs.');
    Exit(Result);
  end;
...
begin
  ...
  try
    try
      ...
      if (lIdhttp.ResponseCode <> 200) or (Trim(lStream.DataString) = '') then
        Exit;
      ...
      begin
        if E.ErrorCode = 403 then
          ShowMessage('A API "' + GetNomeFonte(Fonte) + '" rejeitou o CEP. Código 403 (proibido).')
        else
          ShowMessage('Erro HTTP: ' + IntToStr(E.ErrorCode) + ' - ' + E.Message);
      end;
    end;
  finally
    lStream.Free;
